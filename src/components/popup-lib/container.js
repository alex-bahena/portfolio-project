...
class Container extends Component {
    ...
    if (show) {
      ...
      this.setState(
        merge(currentConfig, {
          ...
          show: true,
          transition: fadeIn
            ? `all ${fadeInSpeed / 1000}s ease-in-out`
            : "none",
          callback: setTimeout(() => {
      ...
      this.setState({
        show: false,
        transition: fadeOut
          ? `all ${fadeOutSpeed / 1000}s ease-in-out`
          : "none",
        callback: setTimeout(() => {
          this.onClosed();
        }, fadeOutSpeed + 1),
  ...
  renderTitleBar() {
    const { className, text, closeText, closeButton, closeButtonClassName } =
      this.state.titleBar;
